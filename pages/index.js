...
export default function Home() {
  ...
  async function loadNFts() {
    ...
    const marketData = Market.networks[networkId]
    ...
    if (nftData && marketData) {
      ...
      const marketAddress = marketData.address
      ...
      window.alert('smart contracts are not deployed on selected network')
    }
  }
            ...
            return (
              <div key={i} className="border shadow rounded-xl overflow-hidden">
                ...
                <div className='p-4 bg-black'>
                  ...
                  <button className="w-full bg-pink-500 text-white front-bold py-2 px-12 rounded" onClick={() => buyNfts(nft)}>Buy</button>
                </div>
              </div>
            )
          })
